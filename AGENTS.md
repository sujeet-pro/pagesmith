...
- `@pagesmith/content` is the primary package and the center of the FS-CMS design.
- `@pagesmith/core` provides markdown rendering, JSX runtime helpers, and runtime CSS.
- `pagesmith` is the optional SSG on top of the content layer.
- Diagram management now routes through `diagramkit` rather than the old in-repo renderer stack.
- Docs live in `docs/` and are built with VitePress.
- AI companion files install through `pagesmith-content ai install`.
