...
- In this repo, the docs config lives at `pagesmith.config.json5` and the root docs content lives at `docs/content/`.
...
- Run `npm run validate:pagesmith` (or `validate:pagesmith:full` for the
  ...
  markdown source under `docs/content/` and the built `gh-pages/`
  output, including:
