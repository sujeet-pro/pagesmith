...
If you need a fully custom site shell, step down to [`@pagesmith/site`](/guide/framework-blog-site). If your framework already owns the entire shell and you only want the headless content layer, use a lower-level `@pagesmith/core` or framework-hosted `@pagesmith/site` integration instead.
