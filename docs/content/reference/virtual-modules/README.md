...
If your app is not using Pagesmith's Vite integration, skip virtual modules and use `createContentLayer()` plus `entry.render()` directly instead. See [Next.js (App Router)](../../guide/framework-nextjs/README.md) for that pattern.
...
![Flowchart showing content.config.ts and content files feeding pagesmithContent(), which generates virtual:content modules and pagesmith-content.d.ts for application imports and editor type checking](./diagrams/virtual-module-flow-light.svg "Flowchart showing content.config.ts and content files feeding pagesmithContent(), which generates virtual:content modules and pagesmith-content.d.ts for application imports and editor type checking")
![Flowchart showing content.config.ts and content files feeding pagesmithContent(), which generates virtual:content modules and pagesmith-content.d.ts for application imports and editor type checking](./diagrams/virtual-module-flow-dark.svg)
