...
![Two configuration surfaces ContentLayerConfig for core collections versus pagesmith.config.json5 for docs site concerns](./diagrams/config-surfaces-core-vs-docs-light.svg "Two configuration surfaces: typed collections and markdown pipeline live in content.config.ts, while the docs preset owns site-level navigation, theme, search, and hosting fields in pagesmith.config.json5.")
![Two configuration surfaces ContentLayerConfig for core collections versus pagesmith.config.json5 for docs site concerns](./diagrams/config-surfaces-core-vs-docs-dark.svg)
...
The `@pagesmith/docs` package is configured through a JSON5 file. See the [Docs Configuration Reference](../docs-config/README.md) for the full specification.
