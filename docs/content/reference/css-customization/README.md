...
The `@pagesmith/docs` theme includes built-in UI for switching both axes (header dropdown + footer selector), with preferences persisted to `localStorage`. See the [Theming](../theming/README.md) reference for the full system, built-in themes, and how to create custom theme variants.
