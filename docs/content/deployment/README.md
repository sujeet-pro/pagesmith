...
<figure>
  <picture>
    <source srcset="./diagrams/basepath-resolution-priority-dark.svg" media="(prefers-color-scheme: dark)">
    <img src="./diagrams/basepath-resolution-priority-light.svg" alt="Fall-through order for resolving basePath from CLI flag through environment variable config file git detection to default root">
  </picture>
  <figcaption>Priority order for resolving <code>basePath</code>: the first set value wins, so CI can pass <code>--base-path</code> or <code>BASE_URL</code> without editing committed config.</figcaption>
</figure>
