...
Syntax highlighting handled by [Expressive Code](https://expressive-code.com/) with dual themes. **All styling and interactivity (including copy buttons) are injected inline** -- no external CSS or JS required.
...
- **Copy button** -- on every code block, handled by Expressive Code inline scripts
