  ...
  function walkSubdir(dir: string) {
    ...
    for (const entry of readdirSync(dir, { withFileTypes: true })) {
      ...
      if (entry.isDirectory()) {
        ...
        if (entry.name.endsWith('.mermaid') || entry.name.endsWith('.excalidraw')) {
          continue
        }
  ...
  async validate(ctx: ValidationContext): Promise<Issue[]> {
    ...
    for (const page of ctx.pageMetas) {
      ...
      for (const file of assetFiles) {
        ...
        if (!normRefs.has(relToArticle) && !normRefs.has('./' + relToArticle)) {
          issues.push({
            file: relToContent,
            severity: 'warn',
        ...
        for (const entry of readdirSync(diagramsDir)) {
          ...
          if (!normRefs.has(lightRef) && !normRefs.has(darkRef)) {
            issues.push({
              ...
              message: `Diagram source not referenced (no ${lightRef} or ${darkRef}) in ${relative(
                ctx.contentDir,
                page.filePath,
              )}`,
            })
