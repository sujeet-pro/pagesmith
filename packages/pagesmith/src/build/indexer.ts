...
export function buildPageTypeData(
  ...
  const typePages = pages.filter((p) => p.slug.startsWith(`/${type}/`) && p.slug !== `/${type}`)
  ...
  if (meta.items && !meta.series) {
    ...
    const remaining = unsorted.filter((u) => !meta.items!.includes(u.slug))
    return {
      type,
...
export function collectTags(pages: PageMeta[]): Map<string, TagPageData> {
  ...
  for (const page of pages) {
    ...
    const entry = {
      ...
      url: page.slug,
      lastUpdatedOn:
        formatDate(page.frontmatter.lastUpdatedOn || page.frontmatter.lastUpdatedDate) || '',
    }

    for (const tag of pageTags) {
      const t = tag.toLowerCase().trim()
      if (!tags.has(t)) {
        tags.set(t, { tag: t, entries: {} })
      }
      const tagData = tags.get(t)!
      if (!tagData.entries[pageType]) tagData.entries[pageType] = []
      tagData.entries[pageType].push(entry)
    }
  }

  // Sort entries within each tag by lastUpdatedOn descending
  const sortDesc = (a: any, b: any) => (b.lastUpdatedOn || '').localeCompare(a.lastUpdatedOn || '')
  for (const tagData of tags.values()) {
    for (const entries of Object.values(tagData.entries)) {
