...
export async function build(config: ResolvedConfig,): Promise<void> {
  ...
  for (const filePath of mdFiles) {
    ...
    const result = await processMarkdown(raw, siteConfig.markdown || {}, {
  ...
  const tagPageCount = await generateTagPages(
    globalIndex, outDir, primaryLayoutsDir, sitemapEntries, {
  ...
  await generateNotFoundPage(
    siteConfig, outDir, primaryLayoutsDir, config.generators.notFoundLayout,
  )
