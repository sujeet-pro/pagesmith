...
For copy-paste prompts and upgrade playbooks, start with `node_modules/@pagesmith/core/ai-guidelines/usage.md` and `node_modules/@pagesmith/core/ai-guidelines/migration.md`.
...
- Code block styling ships in the shared Pagesmith CSS bundles — do NOT add ad-hoc replacement code block CSS or copy-button JS
...
| -------------------------- | -------------------- | ------------------------------------------- |
| `loader`                   | `string \| Loader`   | Built-in loader name or custom loader       |
...
Code block CSS ships in the shared Pagesmith CSS bundles, while syntax token colors are applied inline per rendered block.
...
- Runtime JS provides TOC highlighting and Pagesmith code-block interactivity. Load the shared content runtime for tabs, copy, and collapse behavior.
