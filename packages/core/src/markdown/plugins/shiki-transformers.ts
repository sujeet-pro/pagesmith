...
function el(tag: string, props: Record<string, any>, children: ElementContent[] = []): Element {
  return {
    type: 'element',
    tagName: tag,
    properties: props,
...
function resolveLang(rawLang: string): { canonical: string; display: string; abbr: string } | null {
  if (!rawLang) return null
  const canonical = LANG_CANONICAL[rawLang] || rawLang
  const display = LANG_DISPLAY[canonical] || canonical.charAt(0).toUpperCase() + canonical.slice(1)
  const abbr = LANG_ABBR[canonical] ?? canonical.slice(0, 2).toUpperCase()
...
export function transformerCodeFrame(options: CodeBlockOptions = {}): ShikiTransformer {
  return {
    ...
    root(root: Root) {
      ...
      if (meta.title) {
        headerChildren.push(el('span', { className: ['code-title'] }, [text(meta.title)]))
      }
      ...
      if (showNumbers || hasCollapse) {
        ...
        const totalLines = codeEl.children.filter(
          (c: ElementContent) => c.type === 'element',
        ).length
        ...
        while (idx < codeEl.children.length) {
          ...
          if (collapseStarts.has(lineNum)) {
            ...
            const summary = el('summary', { className: ['collapse-summary'] }, [
              el('span', { className: ['collapse-gutter'] }, [text(`${lineNum}–${actualEnd}`)]),
              el('span', { className: ['collapse-label'] }, [
                text(`${collected} collapsed line${collected === 1 ? '' : 's'}`),
              ]),
            ])

            const detailsContent = el('div', { className: ['collapse-content'] }, collapsedContent)
        ...
        const codeEl = this.code
        codeEl.children = codeEl.children.filter((c: ElementContent) => c.type === 'element')
      }

      // ── Wrap everything in a figure ──
      const pre = root.children.find(
        (c: Root['children'][number]): c is Element => c.type === 'element' && c.tagName === 'pre',
      )
      if (!pre) return
      ...
      const figure = el('figure', figureProps, [header, pre])
...
export function codeBlockTransformers(options: CodeBlockOptions = {}): ShikiTransformer[] {
  return [transformerLineHighlight(), transformerCodeFrame(options)]
}
