...
function isCodeFigure(node: ElementContent): node is Element {
  if (node.type === 'element' && node.tagName === 'figure' && hasClass(node, 'code-figure')) {
    return true
  }
...
function buildTabGroup(figures: Element[], titles: string[]): Element {
  ...
  return el('div', { className: ['code-tabs'] }, [...radios, tabBar, ...panels])
}
...
function processChildren(parent: Element | Root): void {
  ...
  while (i < oldChildren.length) {
    ...
    if (isCodeFigure(node as ElementContent)) {
      const fig = unwrapFigure(node as ElementContent)
      if (extractTitle(fig)) {
        const group = findGroup(oldChildren as ElementContent[], i)
        if (group) {
