...
- Use the `DocHome` fields only on the home page: `layout`, `tagline`, `badge`, `actions`, `hero`, `features`, `install`, `packages`, `codeExample`. See `pagesmith-docs-add-page` SKILL.md for the full field reference (correct names: `actions[].text`/`.link`, `features[].title`/`.details`, `hero.name`/`.text`).
