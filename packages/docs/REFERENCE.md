...
| ----------------------- | ---------------- | ------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--port <number\|auto>` | number \| 'auto' | `server.devPort` (3000 dev) / `server.previewPort` (4000 prv) | Server port. Pass a number, or `auto` to scan upward from 4000 for the first free port. A numeric value forces `strictPort` for that run; `auto` always scans. |
...
| `--log-level <level>`   | string           | `server.logLevel` (default `info`)                            | Server log level (`silent`, `error`, `warn`, `info`, `verbose`)                                                                                                |
...
| `search.enabled`            | `boolean`                                              | `true`                               | Toggle the entire Pagefind integration. When `false`, skip indexing at build time, omit the Pagefind UI script / stylesheet / modal / trigger from every page, and do not emit `<outDir>/pagefind/`.                                                          |
...
| `server.devPort`            | `number \| 'auto'`                                     | `3000`                               | Port for `pagesmith-docs dev`. Pass a number, or `'auto'` to scan upward from `4000` for the first available port at startup. Any string that is not `'auto'` and not a valid port number throws a config error.                                              |
| `server.previewPort`        | `number \| 'auto'`                                     | `4000`                               | Port for `pagesmith-docs preview`. Pass a number, or `'auto'` to scan upward from `4000` for the first available port at startup.                                                                                                                             |
| `server.strictPort`         | `boolean`                                              | `false`                              | When the resolved port is a number and is already in use, fail instead of scanning for the next available port. Ignored when the port is `'auto'`.                                                                                                            |
| `server.logLevel`           | `'silent' \| 'error' \| 'warn' \| 'info' \| 'verbose'` | `'info'`                             | Log level for the dev and preview servers. CLI `--log-level` overrides this value at startup.                                                                                                                                                                 |

Example:

```json5
// All defaults — dev on 3000, preview on 4000, info logs.
{
  /* server omitted */
}
```
...
```json5
// Custom ports + verbose logs; "auto" scans upward from 4000 if 5050 is busy.
{
  server: {
    devPort: 5000,
    previewPort: "auto",
...
The search runtime is always loaded under your `basePath`. Pagesmith emits an explicit `<pagefind-config bundle-path="<basePath>/pagefind/">` element ahead of the Pagefind component-UI loader script so search keeps working on sub-path deploys (GitHub Pages project sites, `/docs/`, etc.). You do not need to set this yourself.

Set `search.enabled: false` to opt out entirely. When disabled, the Pagefind binary is never invoked at build time, the `<outDir>/pagefind/` directory is not produced, and every page is rendered without the Pagefind UI script, stylesheet, modal markup, or trigger button — useful for shaving the index, WASM, and UI bundle when search is not needed.
