...
This file is for fresh setup and retrofit work. For upgrading an existing `@pagesmith/docs` installation and adopting the latest package guidance/features, use `node_modules/@pagesmith/docs/ai-guidelines/migration.md`.
...
7. Prefer the package init flow after installation: run `npx pagesmith init` with explicit values such as `--yes`, `--content-dir`, `--base-path`, `--origin`, and `--ai`. If the init command is not a fit for the repo, perform the same setup manually.
8. Keep pagesmith.config.json5 at the repository root. Set contentDir to the chosen docs folder and set outDir to gh-pages unless the repo already has a better deployment target.
...
11. Use proper frontmatter and meta files. Follow the version-matched schemas in node_modules/@pagesmith/docs/schemas/ for pagesmith.config.json5, meta.json5, and docs frontmatter.
...
14. Update or create root CLAUDE.md and AGENTS.md with these pointers:
    ...
    - For @pagesmith/docs operating guidance, read node_modules/@pagesmith/docs/ai-guidelines/usage.md
...
```text
Improve the existing @pagesmith/docs integration in this repository. Read node_modules/@pagesmith/docs/ai-guidelines/setup-docs.md first and then make the current setup more agent-friendly without breaking the existing docs structure.
...
3. Prefer running `npx pagesmith init` with explicit values to fill missing defaults and starter structure when that is less invasive than hand-editing. Manual edits are also acceptable when the repo is already close to the desired shape.
