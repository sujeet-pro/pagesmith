...
This file is for first-time setup and retrofit work. For upgrades to an existing `@pagesmith/site` integration, use `node_modules/@pagesmith/site/ai-guidelines/migration.md`.
...
```text
Set up `@pagesmith/site` in this repository. Read `node_modules/@pagesmith/site/ai-guidelines/setup-site.md` first and follow it exactly.

Requirements:
1. Install `@pagesmith/core` and `@pagesmith/site` in the relevant root package or workspace package.
2. Inspect the existing app and build system before choosing the integration shape.
3. Keep content collections, markdown rendering, and `pagesmithContent` on `@pagesmith/core`.
...
5. If the repo already owns routing or build tooling (for example Next.js or another framework host), keep that ownership in place and use `@pagesmith/site` only for the shared JSX runtime, CSS bundles, and browser runtime modules that the project actually needs.
...
10. If AI artifacts are missing, install or refresh them with `npx pagesmith-core ai --profile default`. If the project already maintains custom root `llms.txt` files, use `npx pagesmith-core ai --profile default --no-llms`.
...
```text
Improve the existing `@pagesmith/site` integration in this repository. Read `node_modules/@pagesmith/site/ai-guidelines/setup-site.md` first and then make the current setup more agent-friendly without changing the project’s architecture unnecessarily.

Requirements:
1. Keep content-layer ownership in `@pagesmith/core`.
...
4. If the project uses direct Vite plugins, verify `pagesmithContent` stays on `@pagesmith/core/vite` and `pagesmithSsg` / `sharedAssetsPlugin` stay on `@pagesmith/site/vite`.
5. Refresh AI pointers so future agents read `setup-site.md`, `usage.md`, `REFERENCE.md`, and `.pagesmith/markdown-guidelines.md`.
6. Refresh AI artifacts with `npx pagesmith-core ai --profile default` only when they are missing or clearly stale.
